...
namespace SalesEngCustomOrbitActivities
{
    ...
    public sealed class CPTraceActivity : CodeActivity<TraceResult>
    {
        ...
        protected override TraceResult Execute(CodeActivityContext context)
        {
            ...
            double lng = X.Get(context);
            double lat = Y.Get(context);
            return DoTrace(lng, lat);
        }
        ...
        static TraceResult DoTrace(double lng, double lat)
        {
            try
            {
                ...
                string json;
                RootObject o;
                CallArcFMServerTrace(lat, lng, out json, out o);
                ...
                result.Rectifiers = rectifiers.Select(p => (Rectifier)p).ToList();
                result.Anodes = anodes.Select(p => (Anode)p).ToList();
                result.TestPoints = testPoints.Select(p => (TestPoint)p).ToList();
                return result;
            }
            catch (Exception ex)
            {
        ...
        private static void CallArcFMServerTrace(double lat, double lng, out string json, out RootObject o)
        {
            double x = 0; double y = 0;
            ToWebMercatorAuxSphere(lat, lng, out x, out y);
            ...
            string startPoint = x.ToString() + "%2C" + y.ToString();
